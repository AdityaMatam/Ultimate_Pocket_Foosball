...
/**
 * Controls the movement of the ball and handles collisions
 * 
 * @author Aditya Matam
 * 
 */
public class BallMovement {
	static int xC = 140, yC = 266, yV = 0, xV = 0; // Values for the x and y
													// coordinates and
													// velocities
	Ellipse2D.Double b;
	Ellipse2D.Double[] p1 = new Ellipse2D.Double[10]; // An Ellipse2D.Double for
														// each player
	...
	public void updateBallPosition(JLabel ball, JLabel[] player1,
		...
		yC += yV;
		b = new Ellipse2D.Double(xC, yC, 7, 7); // Creates an Ellipse2D.Double
												// so that collisions can be
												// detected easier
		Rectangle2D.Double[] n1 = createNet(net1); // Creates the array of net
													// rectangles
		Rectangle2D.Double[] n2 = createNet(net2);
		for (int x = 0; x < 10; x++) { // Updates the Ellipse2D.Double array so
										// that collisions between players can
	 ...
	 *         detection
	 */
	public Rectangle2D.Double[] createNet(JLabel net) { // Necessary, as the net
														// is divided into 3
														// parts: two side
														// panels that reflect
														// the ball, and the
	...
	public void detectCollision(Rectangle2D.Double[] n1, Rectangle2D.Double[] n2) {
		...
		if (xC < 20 && xV < 0) { // Determines if the ball hits the edges, and
									// if so, inverts approprate velocity
			xV = -xV;
		} else if (xC > 252 && xV > 0) {
			xV = -xV;
		} else if (yC < 89 && yV < 0) {
		...
		for (int x = 0; x < 10; x++) {
			...
			int increaseAmount = 1; // Amount to increase/decrease x and y
									// velocities if it hits the corner of the
									// player
			if (b.intersects(boundingRectp1[2])) {
				...
				if (xV > 1 && (yV >= 1 && yV < 8)) { // Determines which
														// direction the ball
														// was moving when it
														// hit the rectangle, as
					...
					xV -= increaseAmount;
					yV -= increaseAmount;
				} else if (yV <= 0) { // Ensures that the total velocity is
										// always 8
					if (xV < -1) {
						xV += increaseAmount;
						yV -= increaseAmount;
				...
				if (yV >= 0) { // Only inverts the y if the ball is not going in
								// the right direction
					yV = -yV;
				}
			} else if (b.intersects(boundingRectp1[1])) { // Follows same logic
															// as the commented
															// code above
				...
				if (yV <= 0)
					yV = -yV;
			} else if (b.intersects(boundingRectp2[0])) {// Follows same logic
															// as the commented
															// code above
			...
			} else if (b.intersects(boundingRectp2[1])) {// Follows same logic
															// as the commented
															// code above
	 ...
	 */
	public Rectangle2D.Double[] createBoundingRect(Ellipse2D.Double player) { // Required
																				// so
																				// that
																				// collisions
																				// can
																				...
																				// ball
																				// hits
		Rectangle2D.Double[] rect = new Rectangle2D.Double[3]; // Array of the
																// rectangles
																// for the
																// players
	...
	public static void resetBall(boolean global) {
		if (!global) {
			...
			for (int i = 3; i != 0; i--) { // Counts down from 3
		...
		while (xV == 0 || yV == 0) { // So that x or y will never be 0
			xV = (int) (Math.random() * 13 - 6); // Assigns x a random number
													// from -6 to 6
			if (Math.random() < 0.5) { // 50% chance of y being positive or
										// negative
				yV = 8 - Math.abs(xV);
			} else {
				yV = -(8 - Math.abs(xV));
